...
if __name__ == '__main__':
    ...
    for SNRdB in np.arange(0, 6.25, 0.25):           #0, 6.25, 0.25
        ...
        for sig_num in range(rrow):
            ...
            for layer in range(1, p_num+1):                         #中間層ごとの処理(0層目はいらないため, 1層目から最終層まで)
                chead = (layer - 1) * S_REG                         #各層の処理される受信ビットの先頭インデックス
                rcv_sig_ele = rcode[sig_num, chead:chead+S_REG]     #処理される受信ビット
                for node_num in range(nodes_num):                   #ノードごとの処理
                    node = nodes[node_num]                          
